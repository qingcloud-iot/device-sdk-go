...
//snowflake算法生成唯一id
const (
	twepoch        = int64(1552631195000) // 默认起始的时间戳 1552631195000 。计算时，减去这个值
...
func GetInsIdWorker(NodeId int64) *IdWorker {
	once.Do(func() {
		var districtId int64
		districtId = 1 //暂时默认给1 ，方便以后扩展
		idWorker := &IdWorker{}
		if NodeId > maxNodeId || NodeId < 0 {
			panic(fmt.Sprintf("NodeId Id can't be greater than %d or less than 0", maxNodeId))
		}
		if districtId > maxDistrictId || districtId < 0 {
			panic(fmt.Sprintf("District Id can't be greater than %d or less than 0", maxDistrictId))
		}
		...
		worker = idWorker
		logrus.Infof("worker starting. timestamp left shift %d, District id bits %d, worker id bits %d, sequence bits %d, workerid %d", timestampLeftShift, DistrictIdBits, NodeIdBits, sequenceBits, NodeId)
	})
	return worker
}
...
func (id *IdWorker) NextIds(num int) ([]int64, error) {
	if num > maxNextIdsNum || num < 0 {
		logrus.Infof("NextIds num can't be greater than %d or less than 0", maxNextIdsNum)
